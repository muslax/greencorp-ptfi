...
ACCEPTED_QUERIES['get-persepsi'] = async function (req, res) {
  try {
    const { db } = await connect();
    const { rid } = req.query;

    const rs = await db.collection(MONGO_DOC.Persepsi).findOne({ _rid: rid });
...
ACCEPTED_QUERIES['get-lintas-darat'] = async function (req, res) {
  try {
    const { db } = await connect();
    const { rid } = req.query;

    const rs = await db.collection(MONGO_DOC.LintasDarat).findOne({ _rid: rid });
...
ACCEPTED_QUERIES['get-lintas-air'] = async function (req, res) {
  try {
    const { db } = await connect();
    const { rid } = req.query;
